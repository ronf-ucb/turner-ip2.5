...
{ 	CRITICAL_SECTION_START; // Disable interrupts
	...
	radioConfirmationPacket(RADIO_DST_ADDR, CMD_ESTOP, status, length, frame);
    	return; //success     
}
...
// note packet won't be sent until return from battery interrupt
	radioConfirmationPacket(RADIO_DST_ADDR, CMD_ESTOP, status, string_length, message_string);
